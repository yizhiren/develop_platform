...
| 四角色 Pi Agent Core | Agent Runtime | 固定版本 Node 桥接；模型维持连续工具会话，Python RPC 保留路径/命令/Schema/证据门禁；按角色最小工具集 | Node 协议测试、四类 Python 工具循环测试、真实 DeepSeek Agent1 主动读仓测试、完整回归 |
...
| 需求澄清对话 | 产品需求、状态机 | 澄清前只读仓库分析、CI/目标文件证据、结构化问题清单、持久化用户回复、Agent1 上下文回传与规格版本更新 | 工作流顺序与上下文单测、仓库快照选择测试、Agent1 提示词测试、前端构建与页面文案测试 |
| 方案评审与开工授权 | 产品需求、状态机 | 结构化方案评审区；方案置信度严格高于可配置阈值时由平台记录审计并自动开工，否则保留批准/退回门禁；调整意见持久化并回传系统架构师 | 90/91 边界、缺失置信度安全回退、工作流反馈上下文单测、前端构建与页面文案测试 |
...
| Provider 合并兼容门禁 | Git Worker、里程碑 16 | fine-grained PAT 通过 Actions API 按 reviewed SHA 校验 GitHub Actions workflow runs，并合并传统 Commit Status；第三方 Check Run 由 PR clean、reviewed SHA 一致性与 GitHub 分支保护最终裁决 | Actions/Commit Status 映射、403 降级、非 403 拒绝、PR clean、head 漂移拒绝单测；真实 PR #4 squash merge |
