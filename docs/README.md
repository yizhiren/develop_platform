# ForgeFlow 文档索引

## 产品与架构

- [产品需求](product/requirements.md)
- [总体架构](architecture/system.md)
- [需求状态机](architecture/workflow.md)
- [数据模型](architecture/data-model.md)
- [Agent Runtime](architecture/agent-runtime.md)
- [API 约定](api/conventions.md)

## 质量与运维

- [威胁模型](security/threat-model.md)
- [测试策略](testing/strategy.md)
- [开发与运维手册](operations/runbook.md)
- [Git Provider 接入](operations/provider-setup.md)

## 决策与过程

- [ADR 索引](adr/README.md)
- [风险登记表](process/risk-register.md)
- [需求追踪矩阵](process/traceability.md)
- [里程碑 0](process/milestone-0.md)
- [里程碑 1](process/milestone-1.md)
- [里程碑 2](process/milestone-2.md)
- [里程碑 3](process/milestone-3.md)
- [里程碑 4](process/milestone-4.md)

文档使用中文描述产品和决策，接口字段和代码符号使用英文。图表使用 Mermaid，以便与代码一同评审和版本化。
