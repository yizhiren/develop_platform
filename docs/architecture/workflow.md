...
澄清阶段的 `clarification_spec.open_questions` 是交互门禁：Web 直接展示问题清单；`request_more_clarification` 将需求提出者的回答同时写入 `conversation_messages`，并把完整对话与最新规格传给下一轮 Agent1。列表非空时进入 `AWAITING_CLARIFICATION` 等待回答；列表为空时通过 `clarification_complete` 自动进入方案阶段，不再要求用户重复确认。

方案阶段是代码变更前的开工授权门禁。Web 必须在操作按钮之前结构化展示 `architecture_plan` 的目标架构、仓库改动、接口与数据库影响、测试策略、安全风险和回滚方案。`request_plan_change` 必须包含具体意见；意见写入 `conversation_messages`，并连同旧方案传给下一轮系统架构师。
