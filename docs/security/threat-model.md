...
Agent 命令白名单禁止依赖安装、发布、登录和配置动作，子进程不会继承模型或 Git Secret。模型 Key 由 Entrypoint 转为短生命周期文件，Worker 读取后立即删除，并从主进程 exec 环境移除。Agent Worker 可以出网请求 DeepSeek，但仓库测试代码只在通过 Unix Socket 连接的 `network_mode: none` Sandbox Executor 中运行；实测对外 TCP 连接失败。Git Worker 不使用 Agent 接触过的 `.git`：它从控制面保存的 URL fresh clone，只同步拒绝 symlink/特殊文件的普通工作树，再关闭 hooks、扫描 staged Diff 并 Push。
