...
def test_migrations_are_versioned_and_idempotent(tmp_path: Path) -> None:
    ...
    with engine.connect() as connection:
        ...
        assert connection.execute(text("SELECT name FROM schema_migrations WHERE version = 6")).scalar_one() == "agent_run_diagnostics"
        agent_columns = {row[1] for row in connection.exec_driver_sql("PRAGMA table_info(agent_runs)")}
        assert {"error_message", "diagnostics_json"} <= agent_columns
